...
### Changed
- Migrate to OXID eShop 8.0 compatibility
- Update to DBAL 4: `execute()` → `executeQuery()`/`executeStatement()`
- Migrate repositories to QueryBuilderFactory pattern instead of plain Connection
- Update Codeception test configuration for OXID 8.0

### Removed
- Support for OXID eShop 7.x
